...
const EducationCard = ({title, subTitle, Experience, result}) => {
  return (
    <div className='w-full h-1/3 group flex'>
      ...
      rounded-lg p-4 lgl:px-10 flex flex-col justify-center gap-6 lgl:gap-10 shadow-shadowOne'>
        ...
        <p className='text-sm mt-2 text-gray-400 group-hover:text-white duration-300'>{Experience}</p>            
          </div>        
          <div>
          {result && <p  className="px-4 py-2 text-designColor bg-black bg-opacity-25 rounded-lg flex justify-center items-center shadow-shadowOne text-sm font-medium">{result}</p>}
        </div> 
        </div>
